...
const FindNearestShop = ({ nearestShop }) => {
    ...
    useEffect(() => {
        if (nearestShop) {
            map.flyTo(nearestShop.position, 15);
        }
    }, [nearestShop, map]);
...
const ShopLocation = () => {
    ...
    return (
        <div>
            <button onClick={findNearestShop} className="bg-black rounded-b-lg text-white font-poppins z-[9999] absolute right-0 p-[10px]">Nearby Me</button>
